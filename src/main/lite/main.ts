...
let IMPORT_LAYOUT_INPUT: HTMLInputElement;
...
function sanitizeImportedHubState(stateRaw: unknown): HubState | null {
  if (typeof stateRaw !== "object" || stateRaw === null) {
    return null;
  }

  let state = JSON.parse(JSON.stringify(stateRaw)) as HubState;
  if (
    typeof state.sidebar !== "object" ||
    state.sidebar === null ||
    typeof state.tabs !== "object" ||
    state.tabs === null ||
    !Array.isArray(state.tabs.tabs)
  ) {
    return null;
  }

  state.tabs.tabs = state.tabs.tabs.filter((tab) => LITE_COMPATIBLE_TABS.includes(tab.type));
  if (state.tabs.tabs.length === 0) {
    return null;
  }

  if (typeof state.tabs.selected !== "number" || !Number.isFinite(state.tabs.selected)) {
    state.tabs.selected = 0;
  }
  if (state.tabs.selected < 0) {
    state.tabs.selected = 0;
  }
  if (state.tabs.selected >= state.tabs.tabs.length) {
    state.tabs.selected = state.tabs.tabs.length - 1;
  }

  return state;
}

async function handleImportLayoutSelection() {
  const selectedFile = IMPORT_LAYOUT_INPUT.files?.[0];
  IMPORT_LAYOUT_INPUT.value = "";
  ...
  try {
    parsedData = JSON.parse(await selectedFile.text());
  } catch {
    window.alert("Failed to import layout. The selected file could not be parsed as JSON.");
    return;
  }

  if (typeof parsedData !== "object" || parsedData === null) {
    window.alert("Failed to import layout. The selected file was not a recognized format.");
    return;
  }
  ...
  if (!Array.isArray(data.hubs) || !Array.isArray(data.satellites)) {
    const oldLayout = Array.isArray(data.layout);
    window.alert(
      oldLayout
        ? "Failed to import layout. The selected layout file uses an older format which is not compatible with the current version of AdvantageScope."
        : "Failed to import layout. The selected file was not a recognized format."
    );
    return;
  }

  if (data.hubs.length === 0) {
    window.alert("Failed to import layout. The selected layout did not contain any hub state.");
    return;
  }

  if (
    typeof data.version === "string" &&
    data.version !== LITE_VERSION &&
    !window.confirm(
      "The layout file was generated by a different version of AdvantageScope. Compatibility is not guaranteed. Continue importing?"
    )
  ) {
    return;
  }

  const firstHub = data.hubs[0] as { state?: unknown };
  const importedState = sanitizeImportedHubState(firstHub?.state);
  if (importedState === null) {
    window.alert("Failed to import layout. The layout does not contain any tabs supported by AdvantageScope Lite.");
    return;
  }

  localStorage.setItem(LocalStorageKeys.STATE, JSON.stringify(importedState));
  sendMessage(hubPort, "restore-state", importedState);
}
...
/** Opens a file picker to import a layout. */
function openImportLayout() {
  IMPORT_LAYOUT_INPUT.click();
}
...
window.addEventListener("load", () => {
  ...
  IMPORT_LAYOUT_INPUT = document.createElement("input");
  IMPORT_LAYOUT_INPUT.type = "file";
  IMPORT_LAYOUT_INPUT.multiple = false;
  IMPORT_LAYOUT_INPUT.accept = ".json,application/json";
  IMPORT_LAYOUT_INPUT.hidden = true;
  IMPORT_LAYOUT_INPUT.addEventListener("change", () => {
    void handleImportLayoutSelection();
  });
  document.body.appendChild(IMPORT_LAYOUT_INPUT);
